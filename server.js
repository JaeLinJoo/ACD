    ...
    connection.query('SELECT * from team where name = ?', [req.body.teamname], function (err, rows) {
        if (!err) {
            ...
            if (rows[0].mentor == '0' && req.body.isMentor == true) { //멘토가 필요없으면 false
                result = {
                    check: false,
                    message: '멘토가 필요없는 소모임 입니다!'
                };
                res.json(result);
            } else if (lock == 1) { //이미 팀에 속하면 false
                result = {
                    check: false,
                    message: '이미 소모임에 가입 하셨습니다!'
                };
                res.json(result);
                ...
                if (users.length != rows[0].member_count) { //팀등록 성공
                    ...
                    if (req.body.isMentor == true) { //멘토면 멘토등록
                        connection.query('UPDATE team SET mentorname = ? where name = ?', [id, req.body.teamname]);
                        connection.query('UPDATE team SET mentor = ? where name = ?', ['2', req.body.teamname]);
                    }
...
app.post('/uploadAttendImg', multer({
    storage: storage
}).single('image'), function (req, res) {
...
app.get('/getBaIntro/:teamName', function (req, res) {
    var result;
    var teamName = req.params.teamName;
    connection.query('SELECT * from team where state=0 and name=?', [teamName], function (err, rows) {
        if (!err) {
            if (rows[0]) {
                result = {
                    name: rows[0].name,
                    ...
                    admit: rows[0].admit,
                    count: rows[0].member_count,
                    objective: rows[0].objective,
                    objectives: rows[0].objectives
                };
            }
        }
        console.log("getBaIntro---------", result);
        res.json(result);
    })
})
app.get('/getObjSingoHistory', function (req, res) {
    var result = [];
    connection.query('SELECT * from teamobjective  where isreported=?', ['신고'],
        function (err, rows) {
            if (!err) {
                for (i = 0; i < rows.length; i++) {

                    var id = rows[i].id;
                    var objective = rows[i].objective;
                    var name = rows[i].name;
                ...
                console.log("getObjSingoHistory---------", result);
                res.json(result);
            }
        })
})
app.get('/getAttendSingoHistory', function (req, res) {
    var result = [];
    connection.query('SELECT * from teamattend WHERE isreported=?', ['신고'], function (err, rows) {
        if (!err) {
            for (i = 0; i < rows.length; i++) {
            ...
            console.log("getAttendSingoHistory---------", result);
            res.json(result);
        }
    })
})
app.get('/getBeforeApproval', function (req, res) {
    var result = [];
    connection.query('SELECT * from team where state=0', function (err, rows) {
        if (!err) {
            for (i = 0; i < rows.length; i++) {
                result.push({
                    name: rows[i].name
        ...
        res.json(result);
        console.log('getBeforeApproval-----------', result);
    })
})

app.get('/getObjSingoDetail/:teamInfo', function (req, res) {
    var result;
    var data = req.params.teamInfo;
    var st = data.split(",");
    var name = st[0];
    var obj = st[1];
    var id = st[2];
    connection.query('SELECT * FROM teamobjective WHERE name=? AND objective=? AND id=?', [name, obj, id], function (err, rows) {
        if (!err) {
            data = fs.readFileSync(__dirname + "\\uploads\\" + rows[0].img);
            str = data.toJSON().data;
            result = {
                ...
                objective: rows[0].objective,
                img: str,
                reportmsg: rows[0].reportmessage,
                id: rows[0].id
            }
            console.log('getObjSingoDetail---', result);
            res.json(result);
        } else
            console.log('getObjSingoDetail=-------err');
    })
})

app.get('/getAttendSingoDetail/:teamInfo', function (req, res) {
    var result;
    var data = req.params.teamInfo;
    console.log("getAttend", data);
    var st = data.split(",");
    console.log("getAttend", st);
    ...
    user = user.slice(0, -1);
    console.log("getAttend", name, date);
    console.log("getAttend", user);

    connection.query('SELECT * FROM teamattend WHERE name=? AND date=? AND user=?', [name, date, user], function (err, rows) {
        if (!err) {
            data = fs.readFileSync(__dirname + "\\uploads\\" + rows[0].img);
            str = data.toJSON().data;
            result = {
            ...
            console.log('getAttendSingoDetail----', result);
            res.json(result);
        } else
            console.log('getAttendSingoDetail=-------err');
    })
})
app.post('/postApproval', function (req, res) {
    var result;
    var name = req.body.name;
    console.log('postApproval--', name);
    connection.query('UPDATE team SET state = ? where name = ?', ['1', name], function (err, rows) {
        if (!err) {
            result = {
                check: true
            }
        ...
        console.log('postApproval------', result);
        res.json(result);
    })
})

app.post('/postObjSingo', function (req, res) {
    var result;
    ...
    console.log('postObjSingo:: ', name, id, obj);
    if (pass == 'accept') {
        connection.query('UPDATE teamobjective SET isreported=? where name=? and id=? and objective=?', ['신고 승인', name, id, obj]);
        result = {
            check: true
        }
    } else if (pass == 'deny') {
        connection.query('UPDATE teamobjective SET isreported=? where name=? and id=? and objective=?', ['신고 거절', name, id, obj]);
        result = {
            check: false
        }
    }
...
app.post('/postAttendSingo', function (req, res) {
    var result;
    var pass = req.body.pass;
    var name = req.body.name;
    var date = req.body.date;
    console.log('postAttendSingo:: ', name, date);
    if (pass == 'accept') {
        connection.query('UPDATE teamattend SET isreported=? where name=? and date=?', ['신고 승인', name, date]);
        result = {
            check: true
        }
    } else if (pass == 'deny') {
        connection.query('UPDATE teamattend SET isreported=? where name=? and date=?', ['신고 거절', name, date]);
        result = {
            check: false
        }
    }
